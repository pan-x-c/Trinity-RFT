...
class TestQueueBuffer(RayUnittestBaseAsync):
    ...
    async def test_queue_buffer_capacity(self):
        ...
        writer = QueueWriter(config)
        reader = QueueReader(config)
        writer.write([Experience(tokens=torch.tensor([1, 2, 3]), prompt_length=2, info={"model_version": 0, "use_count": 0})])
        writer.write([Experience(tokens=torch.tensor([1, 2, 3]), prompt_length=2, info={"model_version": 1, "use_count": 0})])
        writer.write([Experience(tokens=torch.tensor([1, 2, 3]), prompt_length=2, info={"model_version": 2, "use_count": 0})])
        writer.write([Experience(tokens=torch.tensor([1, 2, 3]), prompt_length=2, info={"model_version": 3, "use_count": 0})])

        # should be blocked
        def write_blocking_call():
            writer.write([Experience(tokens=torch.tensor([1, 2, 3]), prompt_length=2, info={"model_version": 4, "use_count": 0})])
