"""Async SQL database storage using SQLAlchemy async engine."""
...
class AsyncSQLExperienceStorage:
    """Async version of SQLExperienceStorage for native non-blocking DB access."""

    def __init__(self, config: StorageConfig) -> None:
        self.logger = get_logger(f"async_sql_{config.name}")
        self.config = config
    ...
    async def init(self) -> None:
        ...
        self.logger.info(f"Async SQL storage initialized at {self.config.path}")
    ...
    async def write(self, data: List[Experience]) -> None:
        ...
        self.logger.info(f"Write {len(data)} experiences to async SQL storage.")
...
class AsyncSQLTaskStorage:
    """Async version of SQLTaskStorage for native non-blocking DB access."""
    ...
    async def init(self) -> None:
        ...
        self._initialized = True
        self.logger.info(f"Async SQL task storage initialized at {self.config.path}")
