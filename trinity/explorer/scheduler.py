...
class RunnerWrapper:
    ...
    async def _drain_records(self, prefix: str) -> bytes:
        try:
            return await self.rollout_actor.drain_experience_records_bytes.remote(prefix=prefix)
        except Exception as exc:
            self.logger.error("records drain from rollout actor failed: %s", exc)
            return b""

    async def _delete_records(self, prefix: str) -> None:
        try:
            await self.rollout_actor.delete_experience_records.remote(prefix=prefix)
        except Exception as exc:
    ...
    async def run_with_retry(
        ...
        Returns:
            ...
            `List[bytes]`: Serialized recorded experiences drained by Scheduler.
        ...
        experience_payloads = await self._consume_finished_records(task, status)
        return status, experience_payloads, self.runner_id, end_time - start_time
...
class Scheduler:
    ...
    def __init__(
        self,
        config: Config,
        rollout_actors: Dict[int, ActorHandle],
    ):
        self.logger = get_logger(__name__)
        self.config = config
        expected_rollout_actor_ids = set(range(config.explorer.rollout_model.engine_num))
        missing_rollout_actor_ids = expected_rollout_actor_ids.difference(rollout_actors)
        if missing_rollout_actor_ids:
            raise ValueError(
                "Scheduler requires rollout actors for all rollout engines; "
                f"missing engine ids: {sorted(missing_rollout_actor_ids)}"
            )
    ...
    async def _create_runner(
        ...
        runner = RunnerWrapper(
            ...
            config=self.config,
            rollout_actor=self.rollout_actors[
                runner_id % self.config.explorer.rollout_model.engine_num
            ],
        )
    ...
    def task_done_callback(self, async_task: asyncio.Task):
        ...
        if async_task.cancelled():
            ...
            self._accumulate_task_result(task, status, experience_payloads)
            self.busy_runners.pop(runner_id, None)
            self.idle_runners.add(runner_id)
    ...
    def _accumulate_task_result(
        self, task: TaskWrapper, status: Status, experience_payloads: List[bytes]
    ) -> None:
    ...
    def _build_task_result(self, task: TaskWrapper) -> Status:
    ...
    def _emit_task_result(self, task: TaskWrapper) -> None:
        ...
        completed_result = CompletedTaskResult(
            ...
            task_id=task_id,
            status=status,
            experience_payloads=list(task.experience_payloads),
        )
    ...
    async def drain_batch_payload_results(
        ...
        """Drain cached completed statuses and payload chunks for one batch."""
    ...
    async def _get_batch_payload_results(
        ...
        *,
        ...
        """Wait for one batch and drain completed statuses plus payload chunks."""
    ...
    async def get_payload_results(
        ...
        """Wait for one batch and return statuses plus Scheduler-drained payload chunks."""
