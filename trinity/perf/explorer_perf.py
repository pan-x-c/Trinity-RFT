...
def run_explorer_perf(options: ExplorerPerfOptions) -> dict[str, Any]:
    ...
    try:
        ...
        ray.get(explorer_actor.prepare.remote(), timeout=options.timeout)
        startup_time_sec = time.perf_counter() - startup_started_at

        run_started_at = time.perf_counter()
        ray.get(explorer_actor.sync_weight.remote(), timeout=options.timeout)
        ray.get(explorer_actor.explore.remote(), timeout=options.timeout)
        ...
        if explorer_actor is not None and ray.is_initialized():
            try:
                ray.get(explorer_actor.shutdown.remote(), timeout=options.timeout)
