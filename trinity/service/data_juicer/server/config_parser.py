...
class ConfigParser:
    """Parser to convert Trinity data processor configs to DJ configs
    ...
    def __init__(
        ...
        Args:
            data_pipeline_config: All configs of specified data pipeline.
            model_api: The LLM model API instance for config parsing.
            model_name: The name of the LLM model to use.
            validate_config (bool): If execute the config validation check.
    ...
    def _direct_mapping(self, config_path: str) -> Namespace:
        """Direct mapping from RFT config to DJ config"""
        dj_config = prepare_side_configs(config_path)
    ...
    def _agent_based_parsing(self, extra_op_args=None, try_num=3) -> Namespace:
        ...
        logger.warning(
            "Agent-based config parsing is an experimental feature, and only works for Cleaners right now."
        )
        ...
        for _ in range(try_num):
            try:
                ...
                response = self.model_api.chat.completions.create(
                    messages=messages, model=self.model_name
                )
                dj_config = self._parse_llm_response(
                    response.choices[0].message.content, extra_op_args
                )
    ...
    def _parse_llm_response(self, response: str, extra_op_args=None):
